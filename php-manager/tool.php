...
class Tool
{
    ...
    /**
     * Inietta i meta tag SEO (description e keywords) nel template HTML della pagina,
     * risolvendo i placeholder [META_DESCRIPTION] e [META_KEYWORDS].
     *
     * Logica di fallback: se $description o $keywords non sono forniti (null o stringa
     * vuota) si usano i valori generici del torneo, così ogni pagina ha comunque dei
     * meta tag validi anche senza valori specifici.
     *
     * I valori vengono sempre passati per htmlspecialchars: finiscono dentro
     * l'attributo content="..." e possono derivare da contenuti dinamici (es. testo
     * e titolo delle News), quindi vanno neutralizzati per evitare rotture/XSS.
     *
     * @param string      $html        Il template HTML completo della pagina.
     * @param string|null $description Meta description specifica (default: descrizione generica).
     * @param string|null $keywords    Meta keywords specifiche (default: keyword generiche).
     * @return string L'HTML con i placeholder SEO risolti.
     */
    public static function setupSEO(string $html, ?string $description = null, ?string $keywords = null): string
    {
        $descrizioneDefault = 'Benvenuti al Patavium Open, il torneo di tennis ufficiale di Padova. Scopri il programma, acquista biglietti e abbonamenti per le sfide più attese sulla terra rossa.';
        $keywordsDefault    = 'Patavium Open, Tennis, Torneo, Padova, Biglietti, Abbonamenti';

        $description = ($description !== null && trim($description) !== '') ? $description : $descrizioneDefault;
        $keywords    = ($keywords !== null && trim($keywords) !== '') ? $keywords : $keywordsDefault;

        $description = htmlspecialchars($description, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $keywords    = htmlspecialchars($keywords, ENT_QUOTES | ENT_HTML5, 'UTF-8');

        $html = str_replace('[META_DESCRIPTION]', $description, $html);
        $html = str_replace('[META_KEYWORDS]', $keywords, $html);
    ...
    public static function buildHeader(string $currentPage): string
    {
        ...
        <span class="logo-brand">
            <img src="' . $basePath . 'assets/images/logo1.webp" alt="" aria-hidden="true" id="logo-sito" />
            <span>Patavium Open</span>
        </span>

        <button type="button" id="nav-toggle" class="nav-toggle" aria-expanded="false" aria-controls="menu" aria-label="Apri il menu di navigazione">
        ...
        if (self::isLoggedIn()) {
            if (self::isAdmin()) {
                $profileLink = $pagesPath . 'area_admin.php';
                $profileLabel = 'Area Admin';
                $profileKey = 'areaadmin';
            } else {
                $profileLink = $pagesPath . 'area_utente.php';
            ...
            if (strtolower($currentPage) === $profileKey) {
                $headerHtml .= "\n                <li class=\"current-link\" aria-current=\"page\">\n                        <img src=\"{$basePath}assets/images/user.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />{$profileLabel}</li>";
            } else {
                $headerHtml .= "\n                <li><a href=\"{$profileLink}\">\n                        <img src=\"{$basePath}assets/images/user.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />{$profileLabel}</a></li>";
            }

        } else {
            // Utente non loggato: un solo link "Profilo" che porta al Login
            if (strtolower($currentPage) === 'login') {
                $headerHtml .= "\n                <li class=\"current-link\" aria-current=\"page\">\n                        <img src=\"{$basePath}assets/images/user.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />Profilo</li>";
            } else {
                $headerHtml .= "\n                <li><a href=\"{$pagesPath}login.php\">\n                        <img src=\"{$basePath}assets/images/user.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />Profilo</a></li>";
            }
        }

        // Link Carrello (Sempre visibile per tutti)
        if (strtolower($currentPage) === 'carrello') {
            $headerHtml .= "\n                <li class=\"current-link\" aria-current=\"page\">\n                        <img src=\"{$basePath}assets/images/shopping-cart.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />Carrello</li>";
        } else {
            $headerHtml .= "\n                <li><a href=\"{$pagesPath}carrello.php\">\n                        <img src=\"{$basePath}assets/images/shopping-cart.svg\" alt=\"\" aria-hidden=\"true\" class=\"icone-menu\" />Carrello</a></li>";
        }
    ...
    public static function buildFooter(string $currentPage): string
    {
        ...
        // Stato "active": se il link punta alla pagina corrente, gli si applica
        // la classe .current-link (niente sottolineatura, arancione chiaro) e
        // aria-current="page" per l'accessibilità. Altrimenti stringa vuota:
        // i link inattivi restano identici a prima (sottolineati e chiari).
        $activeAttr = static function (string $page) use ($currentPage): string {
            return $page === $currentPage
                ? ' class="current-link" aria-current="page"'
                : '';
        };
        ...
        <div class="footer-grid">
            ...
            <div class="footer-col footer-brand">
                <img src="' . $basePath . 'assets/images/logo1.webp" alt="" aria-hidden="true" width="80" height="80" class="footer-logo-img" />
            ...
            <nav class="footer-col" aria-labelledby="footer-esplora-titolo">
                ...
                <ul>
                    <li><a href="' . $basePath . 'index.php"' . $activeAttr('home') . ' lang="en">Home</a></li>
                    <li><a href="' . $pagesPath . 'biglietti.php"' . $activeAttr('biglietti') . '>Biglietti</a></li>
                    <li><a href="' . $pagesPath . 'news.php"' . $activeAttr('news') . ' lang="en">News</a></li>
                    <li><a href="' . $pagesPath . 'faq.php"' . $activeAttr('faq') . '>Domande frequenti</a></li>
                </ul>
            </nav>
            ...
            <nav class="footer-col" aria-labelledby="footer-legale-titolo">
                ...
                <ul>
                    <li><a href="' . $pagesPath . 'termini.php"' . $activeAttr('termini') . '>Termini e condizioni</a></li>
                    <li><a href="' . $pagesPath . 'privacy.php"' . $activeAttr('privacy') . '>Informativa sulla privacy</a></li>
                </ul>
            </nav>
