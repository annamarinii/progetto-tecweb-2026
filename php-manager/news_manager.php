...
class NewsManager {
    ...
    public static function getNews() {
        ...
        $sql = "SELECT N.idNews, N.titolo, N.testo, N.data_pubblicazione, N.immagine, N.alt_immagine, N.inEvidenza, U.nome, U.cognome
                FROM NEWS N
    ...
    public static function inserisciNews($titolo, $testo, $immagine, $alt_immagine, $idAutore, $inEvidenza) {
        ...
        $conn = DBConnection::getConnessione();
        $sql = "INSERT INTO NEWS (titolo, testo, immagine, alt_immagine, idAutore, inEvidenza) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssii", $titolo, $testo, $immagine, $alt_immagine, $idAutore, $inEvidenza);
        $esito = $stmt->execute();
        $stmt->close();
        return $esito;
    }
    ...
    public static function aggiornaNews($idNews, $titolo, $testo, $immagine, $alt_immagine, $inEvidenza) {
        ...
        $conn = DBConnection::getConnessione();
        if ($immagine != "") {
            $sql = "UPDATE NEWS SET titolo=?, testo=?, immagine=?, alt_immagine=?, inEvidenza=? WHERE idNews=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssii", $titolo, $testo, $immagine, $alt_immagine, $inEvidenza, $idNews);
        } else {
            // L'alt si aggiorna comunque, indipendentemente dal cambio immagine
            $sql = "UPDATE NEWS SET titolo=?, testo=?, alt_immagine=?, inEvidenza=? WHERE idNews=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssii", $titolo, $testo, $alt_immagine, $inEvidenza, $idNews);
        }
        $esito = $stmt->execute();
        $stmt->close();
        return $esito;
    ...
    public static function getUltimeNews($limite = 3) { //per le notizie di pages/index.html
        $conn = DBConnection::getConnessione();

        $sql = "SELECT idNews, titolo, testo, data_pubblicazione, immagine, alt_immagine
                FROM NEWS
